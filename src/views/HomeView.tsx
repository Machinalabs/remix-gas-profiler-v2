...
import { CompilationResult, RemixTxEvent, HighlightPosition, CompilationFileSources } from "@remixproject/plugin-api"
...
export const HomeView: React.FC = () => {
  ...
  const [contractGasCostMap, setContractGasCostMap] = useState(new Map<string, GasCostPerLine[]>()) // to render latest gas costs
  ...
  useEffect(() => {
    if (clientInstance) {
      ...
      clientInstance.on('solidity', 'compilationFinished', (filename: string, source: CompilationFileSources, language: string, data: CompilationResult) => {
          ...
          for (const file of Object.keys(contracts)) {
            for (const contract of Object.keys(contracts[file])) {
              ...
              if (isContractCreation) {
                if (`0x${currentContractEVMData.bytecode.object}` === transaction.input) {
                  ...
                  updateContractAddressesMap((transaction as any).contractAddress, {
                    bytecode: currentContractEVMData.bytecode.object,
                    sourceMap: currentContractEVMData.bytecode.sourceMap,
                    deployedBytecode: currentContractEVMData.deployedBytecode.object,
                    deployedSourceMap: currentContractEVMData.deployedBytecode.sourceMap,
                    sourceCode: originalSourceCode,
                    creationGasProfiling
                  })

                  allFiles.forEach((itemFile) => {
                    const itemsFromItemFile = creationGasProfiling.filter(s => s.fileName === itemFile)
